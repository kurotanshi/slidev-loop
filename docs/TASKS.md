...
- [x] 最小 addon：只含印出 `$nav.currentPage` 的 `global-top.vue` +
      回 `{"ok":true}` 的 `vite.config.ts` middleware
- [x] `playground/slides.md`：3 頁測試 deck，以 workspace dependency 掛載 addon
      （實測 Slidev 52.16.0 對 headmatter 相對 addon 路徑解析不符合本 repo 預期）
- [x] smoke script（`scripts/smoke.ts`）：啟 dev server →
      ...
      package.json 保持誠實）
- [x] **Spike A：addon 的 global-top.vue 是否被聚合。**
      smoke script 的 overlay 檢查即驗證；順便跑 `slidev build`
      確認 `import.meta.env.DEV` 守門後產物行為乾淨。
      失敗 → 改用「使用者專案根目錄 re-export」備案。
- [x] **Spike B：addon 的 vite.config.ts middleware 是否生效。**
      smoke script 的 curl 檢查即驗證。
      失敗 → 改用 `setup/vite-plugins.ts` 備案。
- [x] 記錄驗證時的 @slidev/cli 版本：52.16.0

## Phase 0.5 — 架構文件修正

- [x] 依 spike 結果更新 ARCHITECTURE.md：成功則把「待驗證」改為「已驗證（記錄版本）」；
      失敗則套用對應備案並改寫整合章節
- [x] 定 `engines.slidev` 下限（= spike 驗證版本；Slidev 官方版本宣告慣例）；
...
- [x] comments schema 定稿，**兩個 schema 分開**：client POST payload
...
- [x] `agent-instructions/apply-comments.md` 草案（canonical）：定位規則、
...
- [x] `plugin/store.ts`：comments.json 讀寫、schema 驗證（zod）、原子寫入
...
- [x] `plugin/middleware.ts`：GET / POST / DELETE 處理
...
- [x] `global-top.vue` 最小版：快捷鍵切換留言模式、capture-phase 點擊攔截、
      `prompt()` 輸入留言、POST 寫入。
...
- [x] 極簡留言顯示：載入時 GET 既有留言、依頁碼過濾渲染最簡標記
      （只顯示 `open` — 撐起 PRD 故事 2「重新整理後仍在」與
      故事 3「applied 後不再顯示」；重新整理後生效即可，
      不重整即時消失靠 Phase 2 的 ws 推播）
- [x] apply-comments.md 以 Claude Code SKILL.md 形式手動驗證，
...
- [x] 留言輸入框元件取代 `prompt()`（Vue 元件、Escape 取消、Enter 送出）
- [x] pin 標記打磨：取代 1D 的最簡標記，依 rect 相對座標精確定位、視覺樣式
- [x] comments.json 變更推播：middleware 以 Vite watcher 監看檔案，
      `server.ws.send` 推自訂事件通知 overlay 重新載入
      （agent 標記 applied 後 pin 即時消失，不需重新整理頁面）
- [x] 留言列表側欄：檢視全部 open 留言、單筆撤回（DELETE）
- [x] 留言模式的視覺提示（游標樣式、hover 高亮目標元素）
- [ ] applied/skipped 留言的歷史檢視（顯示 `resolution`，可選）

## Phase 3A — Agent 整合最小矩陣（先證明 canonical → adapter 可行）

- [x] `agent-instructions/create-deck.md`（canonical）：收集 → 提綱 → 生成 → 交接
      四步工作流；slidev 語法部分引用現有成熟 skills，不重造
- [x] sync 腳本最小版：只生成兩家 —
      - [x] Claude Code：project plugin（plugin.json + SKILL.md）
      - [x] Codex：AGENTS.md 區塊 + custom prompts
- [x] `npx slidev-loop init --agents <list>` 最小版：寫入上述兩家轉接檔
- [x] Claude Code、Codex 兩家實測 apply-comments 閉環，記錄行為差異

## Phase 3B — 擴大 agent 矩陣

- [x] sync 腳本擴充：
      - [x] Cursor：.cursor/rules + .cursor/commands
      - [x] Gemini CLI：GEMINI.md + .gemini/commands
      - [x] Copilot：.github/copilot-instructions.md + prompt files
- [x] CI：轉接檔與 canonical 來源一致性檢查（drift check）
- [ ] 至少 Cursor 實測閉環，行為差異記錄到 troubleshooting
- [x] `/apply-comments` 加入可選的 PNG 自我驗證步驟：
      視覺/版面相關留言可 export touched slides 並檢查 PNG；不可驗證時需回報，
      不能只因 `slides.md` 已編輯就標記高風險視覺留言為 applied

## Phase 4 — 發布與相容性

- [ ] npm 發布 `slidev-addon-loop`（keywords: slidev-addon, slidev）
      與 `slidev-loop`（init CLI + canonical 指令）
- [x] CI：Vitest + Phase 0 smoke script 對 @slidev/cli@latest 跑煙霧測試 +
      Phase 3B 的 drift check
- [ ] README 完整化：GIF demo、各家 agent 的安裝段落、troubleshooting
      （spike 備案的啟用方式）
- [ ] 發到 Slidev discussions / awesome-slidev 曝光

## Phase 4.5 — 可選自動套用 watcher（明確 opt-in）

預設產品行為仍是「留言寫入 comments.json，使用者手動請 agent 套用」。
不要讓瀏覽器 addon 或 Vite dev server 自動 spawn agent CLI；那會讓安全邊界、
成本控制與失敗復原都變得不透明。若要提供自動化，必須由使用者在 terminal
明確啟動本機 watcher。

- [ ] 設計 `slidev-loop watch --agent <agent> --root <dir>` UX：
      監看 `.slidev/comments.json`，偵測新增 open comments 後呼叫對應 agent CLI
      或提示使用者確認；預設 dry-run 或 confirmation 策略需在 dogfood 後定案
- [ ] agent adapter 增加「可由 watcher 呼叫」的命令模板：
      Claude Code / Codex / Cursor / Gemini CLI 各自的非互動或半互動 CLI 入口需實測，
      不支援的 agent 必須明確標註 manual-only
- [ ] watcher 需具備基本安全欄位：
      顯示即將執行的命令、輸出 log、避免重入、提供 Ctrl-C 關閉、
      對同一批 open comments 去抖與節流
- [ ] 失敗復原：
      agent 執行失敗不得改寫留言狀態；watcher 應回報錯誤並等待下一次檔案變更或手動重試
- [ ] README/troubleshooting 明確區分 manual mode 與 watch mode；
      manual mode 仍是預設與最低安裝 UX
