...
- **瀏覽器 addon 不自動啟動 agent CLI。** 留言寫入 `.slidev/comments.json`
  後只是建立檔案交接點；使用者仍需在自己的 agent 介面明確執行
  `/apply-comments` 或等價指令。自動套用若要做，必須是使用者主動啟動的
  本機 watcher/daemon，而不是 dev server 或瀏覽器任意 spawn 外部程式。
...
3. 我在 Claude Code 執行 `/apply-comments`，agent 逐筆套用修改、回報每筆的處理結果，
   ...
   MVP 驗收以「重新整理後不再顯示」為準；不重整即時消失屬 Phase 2 的變更推播。
...
7. 我明確在 terminal 啟動 `slidev-loop watch --agent <agent>` 後，
   瀏覽器新增 open 留言會觸發本機 watcher 呼叫對應 agent CLI 套用留言；
   watcher 必須顯示正在執行的命令、保留 log，且提供關閉方式。
...
| 自動呼叫 agent CLI 造成安全與預期外成本 | 高 | 預設只做手動 `/apply-comments` 觸發；若新增自動化，必須由使用者在 terminal 顯式啟動 watcher，並限制可執行 agent、顯示命令與 log |
